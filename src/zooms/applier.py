...
from src.zooms.applier_fusion import (
    _apply_fusion_zoom,
    _safe_zoom,
    _zoom_took,
...
def apply_zooms(
    ...
    for i, (item, meta) in enumerate(zip(appended, zoom_meta)):
        ...
        if seg_fade:  # "Smooth Zoom" → animated Fusion ease (primary path)
            ...
            log.info(
                "Segment %d: attempting Fusion zoom  size=%.3f pan=%.3f tilt=%.3f  "
                "seg_len=%d frames  raw_pan=%.3f raw_tilt=%.3f",
                i, seg_zoom, seg_pan, seg_tilt,
                int(meta.get("seg_len", 1)), raw_pan, raw_tilt,
            )
            try:
                _apply_fusion_zoom(
